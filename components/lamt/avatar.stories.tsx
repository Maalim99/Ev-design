...
import Avatar, { Size } from '@/components/lamt/avatar';
import { Button } from '@/components/ui/button';
import { StoryObj, type Meta } from '@storybook/nextjs';
...
/**
 * LAMT Avatar Component
 *
 * Reusable Avatar component that supports different sizes
 * and renders a user icon when src attribute is not provided.
 * Also shows presence indicator when user is online.
 */
const meta = {
  title: 'LAMT/Avatar',
  component: Avatar,
  ...
  argTypes: {
    src: {
      description:
        'Image source URL. If not provided or invalid, avatar will show a user icon.',
    },
    alt: {
      description: 'Alt text for the image. Shown when image is not available.',
    },
    isOnline: {
      description: 'If true, avatar will display a green presence indicator',
    },
    size: {
      description:
        'Avatar size: sm (28px), md (32px), lg (36px), xl (40px). Default: md',
      control: { type: 'radio' },
      options: sizeOptions,
    },
    onClick: {
