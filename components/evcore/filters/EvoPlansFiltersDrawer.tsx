...
import {
  ...
  type EvoPlansFilterSection,
} from "@/lib/evcore/evoPlansFilterSections";

interface EvoPlansFiltersDrawerProps {
  opened: boolean;
  onClose: () => void;
...
export function EvoPlansFiltersDrawer({
  opened,
  onClose,
  formControl,
  onChangeFilter,
  onResetAll,
}: EvoPlansFiltersDrawerProps) {
  const [searchKeyword, setSearchKeyword] = React.useState("");

  const filteredSections = React.useMemo((): EvoPlansFilterSection[] => {
    if (!searchKeyword.trim()) return EVO_PLANS_FILTER_SECTIONS;
    const kw = searchKeyword.toLowerCase();
    return EVO_PLANS_FILTER_SECTIONS
      .map(section => ({
        ...section,
