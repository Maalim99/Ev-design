...
import {
  ...
  type EvoFilterSection,
} from "@/lib/evcore/evoAccountFilterSections";

interface EvoAccountFiltersDrawerProps {
  opened: boolean;
  onClose: () => void;
...
export function EvoAccountFiltersDrawer({
  opened,
  onClose,
  formControl,
  onChangeFilter,
  onResetAll,
}: EvoAccountFiltersDrawerProps) {
  const [searchKeyword, setSearchKeyword] = React.useState("");

  const filteredSections = React.useMemo((): EvoFilterSection[] => {
    if (!searchKeyword.trim()) return EVO_ACCOUNT_FILTER_SECTIONS;
    const kw = searchKeyword.toLowerCase();
    return EVO_ACCOUNT_FILTER_SECTIONS
      .map(section => ({
        ...section,
