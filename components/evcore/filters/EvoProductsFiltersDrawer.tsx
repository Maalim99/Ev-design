...
import {
  ...
  type EvoProductsFilterSection,
} from "@/lib/evcore/evoProductsFilterSections";

interface EvoProductsFiltersDrawerProps {
  opened: boolean;
  onClose: () => void;
...
export function EvoProductsFiltersDrawer({
  opened,
  onClose,
  formControl,
  onChangeFilter,
  onResetAll,
}: EvoProductsFiltersDrawerProps) {
  const [searchKeyword, setSearchKeyword] = React.useState("");

  const filteredSections = React.useMemo((): EvoProductsFilterSection[] => {
    if (!searchKeyword.trim()) return EVO_PRODUCTS_FILTER_SECTIONS;
    const kw = searchKeyword.toLowerCase();
    return EVO_PRODUCTS_FILTER_SECTIONS
      .map(section => ({
        ...section,
